                        ...
                        {messages.length > 0 ? messages.map(msg => (
                            <div key={msg.id} className={cn("p-4 rounded-lg bg-white/5 transition-all", msg.status === 'Approved' && 'border-l-4 border-green-500')}>
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="font-bold text-secondary">{msg.name} <span className="font-normal text-gray-400">&lt;{msg.email}&gt;</span></p>
                                ...
                                {msg.phone && <p className="text-sm text-gray-400 mt-1">Phone: {msg.phone}</p>}
                            </div>
